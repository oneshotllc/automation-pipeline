...
CHECK=$(cfgv "['ci']['checks'][0]"); export CHECK; ATT_PREFIX=$(cfgv "['repair']['attempts_prefix']")
...
until [ "$($GH api "repos/$REPO/commits/$bad/check-runs" --jq '[.check_runs[]|select(.name==env.CHECK and .status=="completed")]|.[0].conclusion // ""' 2>/dev/null)" = "failure" ] || [ $((SECONDS-T0)) -gt 600 ]; do sleep 20; done
echo "check '$CHECK' on bad commit: $($GH api "repos/$REPO/commits/$bad/check-runs" --jq '[.check_runs[]|select(.name==env.CHECK)]|.[0].conclusion')"
...
until [ -n "$($GH api "repos/$REPO/commits/$new/check-runs" --jq '[.check_runs[]|select(.name==env.CHECK and .status=="completed")]|.[0].conclusion // ""' 2>/dev/null)" ] || [ $((SECONDS-T0)) -gt 2400 ]; do sleep 30; done
concl=$($GH api "repos/$REPO/commits/$new/check-runs" --jq '[.check_runs[]|select(.name==env.CHECK)]|.[0].conclusion'); echo "check '$CHECK' after repair: $concl"
sleep 360
