...
LOG=$HOME/Library/Logs/nodemation/boot.log; mkdir -p "$(dirname "$LOG")"
...
pgrep -f 'Runner.Listener' >/dev/null || { ( cd "$HOME/actions-runner-oneshotllc" && nohup ./run.sh >> "$HOME/Library/Logs/nodemation/runner.log" 2>&1 & ) ; log "github runner started"; }
# 4. report
