...
PY
  ...
  payload=$(python3 -c "import json,sys; ctx=json.loads(sys.argv[1]); ov=json.loads(sys.argv[9]); n=int(ov.get(sys.argv[10], sys.argv[3])); print(json.dumps({'required_status_checks': ({'strict': sys.argv[8]=='true', 'contexts': ctx} if ctx else None), 'enforce_admins': sys.argv[2]=='true', 'required_pull_request_reviews': ({'required_approving_review_count': n, 'dismiss_stale_reviews': sys.argv[4]=='true'} if n > 0 else None), 'restrictions': None, 'required_conversation_resolution': sys.argv[5]=='true', 'allow_force_pushes': sys.argv[6]=='true', 'allow_deletions': sys.argv[7]=='true'}))" "$produced" "$ADMINS" "$REVIEWS" "$DISMISS" "$CONV" "$FORCE" "$DEL" "$STRICT" "$OVERRIDE" "$r")
